...
ge::graphStatus MlaPrologTiling::FillTiling()
{
    ...
    baseParams_->set_qcQrScale(qcQrScale);
    baseParams_->set_kcScale(kcScale);
    if (std::abs(qcQrScale - 1.0f) >= std::numeric_limits<float>::epsilon()) {
        baseParams_->set_isQcQrScaleEnable(1);
    } else {
        baseParams_->set_isQcQrScaleEnable(0);
    }
    if (std::abs(kcScale - 1.0f) >= std::numeric_limits<float>::epsilon()) {
        baseParams_->set_isKcScaleEnable(1);
    } else {
        baseParams_->set_isKcScaleEnable(0);
    }
