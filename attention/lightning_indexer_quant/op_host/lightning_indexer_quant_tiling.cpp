...
ge::graphStatus LIQInfoParser::CheckRequiredInOutExistence() const
{
    ...
    OPS_ERR_IF(opParamInfo_.query_dequant_scale.shape == nullptr,
        OPS_LOG_E(opName_, "Shape of tensor query_dequant_scale is nullptr"),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.query_dequant_scale.desc == nullptr,
        OPS_LOG_E(opName_, "Desc of tensor query_dequant_scale is nullptr"),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.key_dequant_scale.shape == nullptr,
        OPS_LOG_E(opName_, "Shape of tensor key_dequant_scale is nullptr"),
        return ge::GRAPH_FAILED);
    OPS_ERR_IF(opParamInfo_.key_dequant_scale.desc == nullptr,
        OPS_LOG_E(opName_, "Desc of tensor key_dequant_scale is nullptr"),
        return ge::GRAPH_FAILED);
...
ge::graphStatus LIQInfoParser::GetAndCheckAttrParaInfo()
{
    ...
    OPS_ERR_IF(*opParamInfo_.queryQuantMode != 0,
               OPS_LOG_E(opName_, "input attr query_quant_mode only supported 0."),
               return ge::GRAPH_FAILED);

    OPS_ERR_IF(*opParamInfo_.keyQuantMode != 0,
               OPS_LOG_E(opName_, "input attr key_quant_mode only supported 0."),
               return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus LIQInfoParser::GetAndCheckInOutDataType()
{
    ...
    OPS_ERR_IF(inputQType_ != ge::DT_INT8,
               OPS_LOG_E(opName_, "The data types of the input query and key must be int8."),
               return ge::GRAPH_FAILED);

    OPS_ERR_IF(weightsType_ != ge::DT_FLOAT16,
               OPS_LOG_E(opName_, "The data types of the input weights must be float16."),
               return ge::GRAPH_FAILED);

    OPS_ERR_IF(inputQueryScaleType_ != ge::DT_FLOAT16,
...
ge::graphStatus LIQInfoParser::GetAndCheckOptionalInput()
{
    if (kLayout_ == DataLayout::PA_BSND) {
        ...
        OPS_ERR_IF(opParamInfo_.actualSeqLengthsK.desc->GetDataType() != ge::DT_INT32,
...
ge::graphStatus LIQInfoParser::CheckShapeDim()
{
    ...
    OPS_ERR_IF(
        ...
        OPS_LOG_E(opName_, "the dim num of key's shape should be 4"),
        return ge::GRAPH_FAILED);
...
void LIQInfoParser::GenerateInfo(LIQTilingInfo &liqInfo)
{
    ...
    liqInfo.pageAttentionFlag = true;
...
ge::graphStatus TilingForLightningIndexerQuant(gert::TilingContext *context)
{
    OPS_ERR_IF(context == nullptr,
        OPS_REPORT_VECTOR_INNER_ERR("LightningIndexerQuant", "Tiling context is null."),
        return ge::GRAPH_FAILED);
