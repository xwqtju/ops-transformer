...
ASCENDC_TPL_ARGS_DECL(LightningIndexerQuant, // 算子OpType
                      ASCENDC_TPL_DTYPE_DECL(DT_Q, LI_TPL_IN8),
                      ASCENDC_TPL_DTYPE_DECL(DT_K, LI_TPL_IN8),
                      ASCENDC_TPL_DTYPE_DECL(DT_OUT, LI_TPL_INT32),
                      ASCENDC_TPL_BOOL_DECL(PAGE_ATTENTION, 1), 
                      ASCENDC_TPL_UINT_DECL(Q_LAYOUT_T, ASCENDC_TPL_4_BW, ASCENDC_TPL_UI_LIST, LIQ_LAYOUT_BSND,
                                            LIQ_LAYOUT_TND),
                      ASCENDC_TPL_UINT_DECL(K_LAYOUT_T, ASCENDC_TPL_4_BW, ASCENDC_TPL_UI_LIST, LIQ_LAYOUT_PA_BSND), );
 
// 支持的模板参数组合
// 用于调用GET_TPL_TILING_KEY获取TilingKey时，接口内部校验TilingKey是否合法
ASCENDC_TPL_SEL(ASCENDC_TPL_ARGS_SEL(ASCENDC_TPL_DTYPE_SEL(DT_Q, LI_TPL_IN8),
                                     ASCENDC_TPL_DTYPE_SEL(DT_K, LI_TPL_IN8),
