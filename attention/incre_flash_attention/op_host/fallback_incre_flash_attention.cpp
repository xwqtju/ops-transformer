...
graphStatus IncreHostExecuteFunc(OpExecuteContext *host_api_ctx)
{
    ...
    OP_CHECK_IF(blockSize == nullptr,  OP_LOGE(host_api_ctx->GetNodeName(),  "blockSize is null"), return GRAPH_FAILED);
    const uint32_t *innerPrecise = attrs->GetAttrPointer<uint32_t>(INNER_PRECISE_INDEX);
    OP_CHECK_IF(innerPrecise == nullptr, OP_LOGE(host_api_ctx->GetNodeName(), "innerPrecise is null"), return GRAPH_FAILED);
    
    OP_CHECK_IF(scaleValue == nullptr,  OP_LOGE(host_api_ctx->GetNodeName(),  "scaleValue is null"), return GRAPH_FAILED);
