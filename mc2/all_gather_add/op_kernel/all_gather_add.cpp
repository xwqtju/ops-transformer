...
#define INVOKE_ALL_GATHER_MATMUL_OP_IMPL(templateClass, ...)                                   \
    do {                                                                                       \
        using aType = AddType<AscendC::TPosition::GM, CubeFormat::ND, A_DTYPE, true>;       \
        using cType = AddType<AscendC::TPosition::GM, CubeFormat::ND, C_DTYPE>;             \
    ...
    GM_ADDR gatherOut, GM_ADDR workspaceGM, GM_ADDR tilingGM)
{
    ...
    if (TILING_KEY_IS(110)) {
        ...
        using bType = AddType<AscendC::TPosition::GM, CubeFormat::NZ, B_DTYPE, true>;
        using biasType = AddType<AscendC::TPosition::GM, CubeFormat::ND, typename BiasType<BIAS_DTYPE>::type>;
        INVOKE_ALL_GATHER_MATMUL_OP_IMPL(AllGatherAddFullMesh, true, false);
    } else if (TILING_KEY_IS(100)) {
        // full mesh + no nd2nz + no bias cast
        using bType = AddType<AscendC::TPosition::GM, CubeFormat::ND, B_DTYPE, true>;
        using biasType = AddType<AscendC::TPosition::GM, CubeFormat::ND, typename BiasType<BIAS_DTYPE>::type>;
        INVOKE_ALL_GATHER_MATMUL_OP_IMPL(AllGatherAddFullMesh, false, false);
    } else if (TILING_KEY_IS(111)) {
        // full mesh + nd2nz + bias cast
        using bType = AddType<AscendC::TPosition::GM, CubeFormat::NZ, B_DTYPE, true>;
        using biasType = AddType<AscendC::TPosition::GM, CubeFormat::ND, float>;
        INVOKE_ALL_GATHER_MATMUL_OP_IMPL(AllGatherAddFullMesh, true, true);
    } else if (TILING_KEY_IS(101)) {
        // full mesh + no nd2nz + bias cast
        using bType = AddType<AscendC::TPosition::GM, CubeFormat::ND, B_DTYPE, true>;
        using biasType = AddType<AscendC::TPosition::GM, CubeFormat::ND, float>;
        INVOKE_ALL_GATHER_MATMUL_OP_IMPL(AllGatherAddFullMesh, false, true);
    }
}