...
static aclnnStatus CheckParams(const aclTensor *x1, const aclTensor *x2, const aclTensor *bias,
  ...
  if (bias != nullptr) {
   OP_LOGW("AllGatherAdd, The current version does not support the scenario where bias is not 0. "
...
static aclnnStatus DealWithX1Empty(uint64_t *workspaceSize, aclOpExecutor **executor) {
  OP_LOGD("AllGatherAdd, dealing with empty tensor.");
  // 固定写法，创建OpExecutor
...
aclnnStatus aclnnAllGatherAddGetWorkspaceSize(const aclTensor *x1, const aclTensor *x2, const aclTensor *bias,
  ...
  aclnnStatus ret = aclnnInnerAllGatherAddGetWorkspaceSize(x1, x2, bias, group, transposeX1, transposeX2,
